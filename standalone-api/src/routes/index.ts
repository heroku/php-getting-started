...
router.post('/index/:project/:collection', authMiddleware.requireKey('write'), zValidator('json', upsertDocSchema), async (c) => {
    try {
        ...
        for (const doc of documents) {
            try {
                ...
                console.error(`Failed to upsert document ${
                    doc.id
                }:`, error);
                results.push({
                    id: doc.id,
...
router.delete('/index/:project/:collection', authMiddleware.requireKey('write'), zValidator('json', deleteDocSchema), async (c) => {
    try {
        const {project, collection} = c.req.param();
...
// Get collection info
router.get('/index/:project/:collection', authMiddleware.requireKey('read'), async (c) => {
    try {
        const {project, collection} = c.req.param();
